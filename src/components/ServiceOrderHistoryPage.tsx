    ...
    ;(async () => {
      try {
        ...
        if (!response.available) {
          setReason(response.reason || 'Histórico de OS indisponível.')
          return
        }
        ...
        if (alive) setReason(e?.message || 'Falha ao consultar histórico de OS.')
      } finally {
  ...
  return <div className="history-page">
    <header className="coupling-hero">
      <div className="brand-area"><img src="/transmassa-logo.png" alt="Transmassa"/><div>
        ...
        <p>Fonte: banco operacional da Transmassa via backend.</p>
      </div></div>
    ...
    <main className="history-content">
      ...
      <section className="history-table-section">
        ...
        {loading ? <div className="history-empty">Consultando histórico de OS no banco...</div> : reason ?
          <div className="history-empty history-warning"><ShieldAlert/><div><strong>Histórico de OS indisponível.</strong><p>{reason}</p></div></div> :
          <div className="os-ledger">
            ...
            {!filtered.length && <div className="history-empty">Nenhuma OS retornada para os veículos atuais.</div>}
          </div>}
      </section>
    </main>
  </div>
